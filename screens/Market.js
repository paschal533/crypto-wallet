...
import { icons, COLORS, SIZES, dummyData, FONTS, constants } from '../constants';
    ...
    return (
        <View style={{
            ...
            {marketTabs.map((item, index) => {
                return (
                    <TouchableOpacity
                        key={`MarketTabe-${index}`}
                        style={{
                            flex: 1,
                        }}
                       onPress={() => onMarketTabPress(index)}
                    >
                        <View ref={item.ref} style={{
...
const Market = ({ getCoinMarket, coins }) => {
    ...
    const marketTabScrollVieRef = React.useRef()

    const onMarketTabPress = React.useCallback(marketTabIndex => {
        marketTabScrollVieRef?.current?.scrollToOffset({
